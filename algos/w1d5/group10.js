...
class SLL {
    ...
    printSecondToLastValue() {
        // counter that has something do with this.length;
        // need a runner to traverse the list
        // if the list is not long (e.g. this.length < 2), then exit the function orconsole.log("not long enough")
        if (this.length < 2) {
    ...
    reverse() {
        ...
        while (temp) {
            // delete each node
            this.delete()
                // add each deleted node to the front, until we reach our original first node (temp)
        }
    }
    ...
    reverse2() {
        ...
        var runner = this.head;
        while (runner) {
            this.addDataToFront(runner.data);
            console.log("adding " + runner.data);
            runner = runner.next;
        }
        var runnerD = this.head;
        while (count > 1) {
            count--;
            runnerD = runnerD.next;
        }
        runnerD.next = null;
    }
