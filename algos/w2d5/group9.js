...
function isStackSorted(stack) {
    ...
    // pop from one stack into container
    // check each node's data to see if the nodes are in order

    // return original stack

    let temp = stack.peek().data;
    let isSorted = true;
    let isSorted2 = true;
    ...
    while (!stack.isEmpty()) {
        ...
        if (!stack.isEmpty()) {
            if (stack.peek().data < temp) {
                isSorted = false;
            }
            temp = stack.peek().data;
        }
    }
    while (!newStack.isEmpty()) {
        stack.push(newStack.pop());
        if (!newStack.isEmpty()) {
            if (newStack.peek().data < temp) {
                isSorted2 = false;
            }
            temp = newStack.peek().data;
        }
    }
...
function greaterOfTwoStacks(stack1, stack2) {
    let stack1Total = totalStack(stack1);
    let stack2Total = totalStack(stack2);

    if (stack1Total > stack2Total) {
        return stack1;
    } else {
        return stack2;
    }
