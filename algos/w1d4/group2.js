...
class SLL {
    ...
    delete(data) {
        ...
        } else if (this.head.data == data) { // if you have to delete the head node
            this.head = this.head.next;
        } else { // loop through list, checking the value of runner.next
            var runner = this.head;
            while (runner.next) {
                if (runner.next.data == data) {
                    runner.next = runner.next.next; // if found, make runner.next point to the value after in order to skip the node
                    return;
                }
                runner = runner.next;
            }
            console.log("value not found"); // if loop traverses entire list and does not find data
        }
    }
    ...
    size() {
        var count = 0;
        var runner = this.head;
        while (runner) { // run through list using while(runner) loop, increment count by 1 each time
            count++;
            runner = runner.next;
        }
        return count;
